...
Uses @craft-agent/mermaid parser for accurate validation.`,
    ...
    {
      name: 'source_create',
      description: `Create a source folder and config.json deterministically.

Creates:
- sources/{slug}/config.json
- sources/{slug}/guide.md

After creating, run source_test to validate the source.`,
      inputSchema: {
        type: 'object' as const,
        properties: {
          name: {
            type: 'string',
            description: 'Human-readable source name (e.g., "Linear")',
          },
          type: {
            type: 'string',
            enum: ['mcp', 'api', 'local'],
          ...
          provider: {
            type: 'string',
            description: 'Provider label (e.g., "linear", "google", "custom")',
          },
          slug: {
            type: 'string',
            description: 'Optional slug override; autogenerated from name when omitted',
          },
          enabled: {
            type: 'boolean',
            description: 'Whether source should be enabled immediately (default true)',
          },
          tagline: {
            type: 'string',
            description: 'Optional tagline shown in UI',
          },
          icon: {
            type: 'string',
            description: 'Optional icon (emoji or URL)',
          },
          mcp: {
            type: 'object',
            properties: {
              ...
              url: { type: 'string' },
              authType: { type: 'string', enum: ['oauth', 'bearer', 'none'] },
          ...
          api: {
            ...
            properties: {
              baseUrl: { type: 'string' },
              authType: { type: 'string', enum: ['bearer', 'header', 'query', 'basic', 'none'] },
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      switch (name) {
        ...
        case 'source_create':
          return await handleSourceCreate(ctx, toolArgs as {
            name: string;
            type: 'mcp' | 'api' | 'local';
            provider?: string;
            slug?: string;
