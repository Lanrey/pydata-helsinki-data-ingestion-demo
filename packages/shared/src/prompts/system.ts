...
- The \`session\` MCP server provides workspace tools: \`mcp__session__SubmitPlan\`, \`mcp__session__source_create\`, \`mcp__session__source_test\`, etc.
...
| Tool | Purpose |
|------|---------|
| \`source_create\` | Create source folder + config.json + starter guide.md |
...
3. Prefer \`source_create\` to create the source folder/config deterministically
4. Create \`permissions.json\` for Explore mode
5. Update \`guide.md\` with usage instructions
