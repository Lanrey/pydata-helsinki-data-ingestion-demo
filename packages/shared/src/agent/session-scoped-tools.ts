 ...
 * - source_create: Create a source folder/config deterministically
...
const sourceCreateSchema = {
  name: z.string().describe('Human-readable source name (e.g., "Linear")'),
  type: z.enum(['mcp', 'api', 'local']).describe('Source type'),
  provider: z.string().optional().describe('Provider label (e.g., "linear", "google", "custom")'),
  slug: z.string().optional().describe('Optional slug override; autogenerated from name when omitted'),
  enabled: z.boolean().optional().describe('Whether source should be enabled immediately (default true)'),
  tagline: z.string().optional().describe('Optional tagline shown in UI'),
  icon: z.string().optional().describe('Optional icon (emoji or URL)'),
  mcp: z.object({
...
**Reference:** ${DOC_REFS.sources}`,
  ...
  source_create: `Create a source folder and config.json deterministically.

Use this instead of ad-hoc file creation when adding a new source.

**Creates:**
- sources/{slug}/config.json
- sources/{slug}/guide.md (starter template)

**Then:** run \`source_test\` once, and if needed trigger the appropriate auth tool.
...
export function getSessionScopedTools(
  ...
  const tools = [
    ...
    // source_create
    tool('source_create', TOOL_DESCRIPTIONS.source_create, sourceCreateSchema, async (args) => {
      const result = await handleSourceCreate(ctx, args as {
        name: string;
        type: 'mcp' | 'api' | 'local';
        provider?: string;
        slug?: string;
        ...
        local?: { path: string; format?: string };
      });
      return convertResult(result);
    }),
