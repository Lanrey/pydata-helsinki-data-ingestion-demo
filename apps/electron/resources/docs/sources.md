...
Prefer `source_create` to create source folders/config deterministically:

```
mcp__session__source_create({ name: "Linear", type: "mcp", provider: "linear", mcp: { transport: "http", url: "https://mcp.linear.app", authType: "oauth" } })
```
...
Agent: [Creates source via source_create with icon/tagline/OAuth settings]
       mcp__session__source_create({ name: "Linear", type: "mcp", provider: "linear", ... })
...
1. Create the source folder:
  - Preferred: call `source_create` with full source configuration
  - Alternative: manually create folder + `config.json`

2. Write `guide.md` tailored to user's context and use case

3. **Create `permissions.json` for Explore mode** - List the source's tools, identify read-only operations (list, get, search), and add simple patterns. Patterns are auto-scoped to this source.

4. Run `source_test` to validate configuration and test connection

5. If auth is required, trigger the appropriate flow:
...
6. Confirm with user that the source is working as expected
